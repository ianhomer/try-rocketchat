...
app.use((req, res, next) => {
	res.set('Access-Control-Allow-Origin', 'http://chat.local:3000'); // this is the rocket.chat URL
...
app.post('/sso', function (req, res) {
	...
	if (!currentUsername) {
		axios.post('http://chat.local:3000/api/v1/users.register', {
			username: 'new-user',
			...
			if (response.data.success) {
				return axios.post('http://chat.local:3000/api/v1/login', {
					username: 'new-user',
		...
		axios.post('http://chat.local:3000/api/v1/login', {
...
app.post('/login', function (req, res) {
	...
	// after user is authenticated we can proceed with authenticating them on rocket.chat side
